...
const offerTypePaneStyle = createStyles(
  (theme, { backgroundColor, isSelected }: OfferTypePaneStyleProps) => ({
    selected: {
      display: 'flex',
      height: calcRem(100),
      width: '33%',
...
const OfferTypePane: FC<OfferTypePaneProps> = ({
  offerType,
  selectedOfferType,
  setOfferType,
}) => {
  ...
  const tooltipText: Map<OFFER_TYPE, string> = new Map<OFFER_TYPE, string>([
    [
      OFFER_TYPE.BUY,
      t('buyTypeExplain', { offerType: getI18OfferTypeName(offerType) }),
    ],
    [
      OFFER_TYPE.SELL,
      t('sellTypeExplain', { offerType: getI18OfferTypeName(offerType) }),
    ],
    [
      OFFER_TYPE.EXCHANGE,
      t('exchangeTypeExplain', { offerType: getI18OfferTypeName(offerType) }),
    ],
  ]);
  ...
  return (
    <Tooltip
      label={tooltipText.get(offerType)}
      position={'bottom'}
      multiline={true}
    >
      <Flex className={classes.selected}>
        <Flex onClick={() => setO()} className={classes.container}>
          <div className={classes.info}>
            <IconInfoCircle size={20} color={'white'} />
          </div>
          <Text color={'white'} fw={700}>
            {getI18OfferTypeName(offerType)}
          </Text>
        </Flex>
      </Flex>
    </Tooltip>
