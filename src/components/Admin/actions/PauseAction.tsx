...
export const PauseAction = () => {
  ...
  const getContratPauseStatus = () =>
    realTokenYamUpgradeable ? realTokenYamUpgradeable?.paused() : false;

  const { data, isLoading } = useQuery(
    ['contractPaused'],
    getContratPauseStatus,
    { enabled: !!realTokenYamUpgradeable },
  );
  useEffect(() => {
    if (data) setIsPaused(data);
  }, [data]);
