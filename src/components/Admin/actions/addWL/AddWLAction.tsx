...
export const AddWLAction = () => {
  ...
  const whitelistToken = async () => {
    try {
      ...
      const tx = await realTokenYamUpgradeable.toggleWhitelistWithType(
        addresses,
        types,
      );
      ...
      /* eslint-disable */
      tx.wait().then(newFunction(notificationPayload));
    } catch (err) {
      console.log('Error while adding token to WL: ', err);
    }

    function newFunction(notificationPayload: {
      key: any;
      href: string;
      hash: any;
    }): any {
      return function (tt: Truc): void {
        updateNotification(
          //TODO: add translation
          NOTIFICATIONS[
            tt.status === 1
